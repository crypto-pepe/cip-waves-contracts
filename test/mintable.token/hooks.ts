...
import { setContract } from '../../steps/mintable.token';
import {
...
export const mochaHooks = async (): Promise<Mocha.RootHookObject> => {
  return {
    async beforeAll(this: Mocha.Context) {
      ...
      contracts.push(
        await deployMultisigContract(init_contracts, 'mintable_token', rootSeed)
      );
      setContract(contracts.filter((f) => f.name == 'mintable_token')[0]);
